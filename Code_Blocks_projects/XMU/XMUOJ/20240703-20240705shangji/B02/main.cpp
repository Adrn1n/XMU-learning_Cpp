...
image.png (./3ec4db1361.png)-----image.png (./f48cf16664.png)
...
1）‘.’：代表紫色迷雾image.png (./668de4a15e.png)；

2）‘#’：代表红墙image.png (./65762c10c1.png)；

3）‘@’：代表林克的起始位置image.png (./bb1c64a143.png)(该字符在每个数据集合中唯一出现一次。）
...
实在搞不定的话，你可以 点此下载测试数据 (./problem_4303_test_cases.zip)
...
short Max_Clean(char A[],const short W,const short H)
{
    if((W>0)&&(H>0))
    {
        char *ptr=A;
        ...
        {
            *ptr='#';
            short res=1,x=(short)((ptr-A)%W+1),y=(short)((ptr-A)/W+1);
            if((x>1)&&(*(ptr-1)=='.'))
            {
                *(ptr-1)='@';
                res+=Max_Clean(A,W,H);
